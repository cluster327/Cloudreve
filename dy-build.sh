...
buildDebugBinary () {
  cd $REPO
  out="debug/cloudreve"
  go build -gcflags "all=-N -l" -a -o "${out}" -ldflags " -X 'github.com/cloudreve/Cloudreve/v3/pkg/conf.BackendVersion=$VERSION' -X 'github.com/cloudreve/Cloudreve/v3/pkg/conf.LastCommit=$COMMIT_SHA'"
}
...
assetRun () {
  ...
  cd $REPO
  rm -rf debug/statics
  cp -r assets/build debug/statics
  ./debug/cloudreve
}
